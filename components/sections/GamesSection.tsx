...
export function GamesSection() {
  ...
  return (
    <section id="attivita" className="relative scroll-mt-24 overflow-hidden bg-charcoal px-5 py-24 text-white sm:px-8 sm:py-32 lg:px-12 lg:py-40">
      ...
      <div className="relative mx-auto max-w-[1600px]">
        <SectionLabel index="02" light>Le tre attività</SectionLabel>
        <div className="mt-6 grid gap-10 lg:grid-cols-[1.25fr_.75fr] lg:items-end">
          <AnimatedHeading className="max-w-6xl text-[clamp(3.5rem,7.6vw,8rem)] font-semibold leading-[0.88] tracking-[-0.065em]">
            Tre momenti.
            <span className="block font-serif font-normal italic text-fire">Un’unica live experience.</span>
          </AnimatedHeading>
        ...
        <div className="mt-16 grid gap-4 md:grid-cols-3 lg:mt-24 lg:pb-10">
          {activities.map((activity, index) => (
            <motion.article
              key={activity.title}
              className={`relative flex min-h-[34rem] flex-col overflow-hidden rounded-[2rem] border border-white/14 bg-white/[0.045] p-7 sm:p-8 ${index === 1 ? "lg:mt-10" : ""}`}
              whileHover={reduceMotion ? undefined : { y: -8 }}
              transition={{ duration: 0.35 }}
            >
              <div className="flex items-start justify-between gap-4">
                <span className="font-serif text-6xl italic leading-none text-fire/80">0{index + 1}</span>
                <span className="rounded-full border border-white/16 px-3 py-1.5 text-[0.58rem] font-bold uppercase tracking-[0.15em] text-white/50">
                  {activity.eyebrow}
                </span>
              </div>
              <h3 className="mt-12 text-[clamp(2.2rem,3.5vw,3.6rem)] font-semibold leading-none tracking-[-0.05em]">{activity.title}</h3>
              <p className="mt-5 text-sm leading-6 text-white/60">{activity.description}</p>
              <ul className="mt-auto space-y-3 border-t border-white/14 pt-7 text-sm" aria-label={`Cosa comprende ${activity.title}`}>
                {activity.highlights.map((highlight) => (
                  <li key={highlight} className="flex items-start gap-3">
                    <span className="mt-[0.45rem] size-1.5 shrink-0 rounded-full bg-fire" aria-hidden="true" />
                    <span>{highlight}</span>
                  </li>
                ))}
              </ul>
